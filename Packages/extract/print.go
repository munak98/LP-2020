...
func PrintYearData(year Year) {
	...
	for i := range year.Races {
		fmt.Printf("\t%s: %d\n", year.Races[i].Name, year.Races[i].Total)
	}
	fmt.Printf("\n*****************************************\n")
	fmt.Printf("Numero de participantes de cada Tipo de Escola:\n")
	for i := range year.Recurse.Schools {
		fmt.Printf("\t%s: %d\n", year.Recurse.Schools[i], year.SchoolTypes[i])
	}
...
func PrintUFRacesMeanScores(state State) {
	for i := range state.Races {
		...
		fmt.Printf("Médias: \n\t%s: %.2f \n\t%s: %.2f \n\t%s: %.2f\n\t%s: %.2f\n\n",
			state.Recurse.Subjects[0], state.Races[i].Medias[0],
			state.Recurse.Subjects[1], state.Races[i].Medias[1],
			state.Recurse.Subjects[2], state.Races[i].Medias[2],
			state.Recurse.Subjects[3], state.Races[i].Medias[3],
		)
...
func PrintUFRaceMeanScores(race Race) {
	...
	fmt.Printf("\nTotal de participantes: %d\n", race.Total)
	fmt.Printf("Médias: \n\t%s: %.2f \n\t%s: %.2f \n\t%s: %.2f\n\t%s: %.2f\n\n",
		race.Recurse.Subjects[0], race.Medias[0],
		race.Recurse.Subjects[1], race.Medias[1],
		race.Recurse.Subjects[2], race.Medias[2],
		race.Recurse.Subjects[3], race.Medias[3],
	)
	fmt.Printf("Número de participantes por tipo de Escola:\n")
...
func PrintUFSchoolMeanScores(state State, school int) {
	...
	fmt.Printf("\nTotal de participantes da escola do tipo %s: %d\n", state.Recurse.Schools[school], state.SchoolType[school])
	fmt.Println("Médias:")
	fmt.Printf("\t%s: %.2f \n\t%s: %.2f \n\t%s: %.2f\n\t%s: %.2f\n\n",
		state.Recurse.Subjects[0], state.SchoolMeanScores[school][0],
		state.Recurse.Subjects[1], state.SchoolMeanScores[school][1],
		state.Recurse.Subjects[2], state.SchoolMeanScores[school][2],
		state.Recurse.Subjects[3], state.SchoolMeanScores[school][3],
	)	
}
